...
- Fixed Alibaba DashScope (Bailian) compatible-mode endpoint `400 InternalError.Algo.InvalidParameter: The provided messages input is invalid. The error info is [Unexpected item type in content.]` when a screenshot or other image-producing tool result was folded into a text-only Qwen turn (e.g. `qwen3.7-max`, `qwen-max`, `qwen3-coder-*`) hosted at `dashscope.aliyuncs.com/compatible-mode/v1`. `convertMessages` in `openai-completions` no longer forwards `image_url` content parts for non-`vl` / non-`omni` / non-`audio` Qwen ids on that endpoint even when a misconfigured custom provider claims `input: ["text", "image"]`; the tool-result branch and the user-content branch both fall back to the standard `[image omitted: model does not support vision]` placeholder so the model still sees the attachment intent. The `coding-intl.dashscope.aliyuncs.com` coding-plan endpoint (whose `qwen3.5-plus`/`qwen3.6-plus` SKUs are genuinely multimodal) is unaffected. ([#1859](https://github.com/can1357/oh-my-pi/issues/1859))
